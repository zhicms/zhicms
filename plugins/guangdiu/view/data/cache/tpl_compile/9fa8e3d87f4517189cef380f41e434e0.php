...
<main class="gd-main">
  <div class="gd-wrap">
    <div class="gd-content">
      <div class="gd-box gd-article">
        <h1 class="gd-article-title"><?php echo tpl_raw($article['title']); ?></h1>
        <div class="gd-article-meta">
          <span>人气：<?php echo tpl_raw($article['view']); ?></span>
          <span><?php echo tpl_raw((isset($article['addtime']) && ($article['addtime'] !== '')?$article['addtime']:'')); ?></span>
          <span>来源：<?php echo tpl_raw((isset($article['source']) && ($article['source'] !== '')?$article['source']:$site_name)); ?></span>
        </div>
        <div class="gd-article-content"><?php echo tpl_raw($article['content']); ?></div>
      </div>

      <?php if(!empty($related)): ?>
      <div class="gd-box">
        <div class="gd-sectitle"><h3>相关推荐</h3></div>
        <div class="gd-list">
          <?php foreach($related as $item): ?>
          <div class="gd-post">
            <a class="gd-post-img" href="<?php echo tpl_raw($item['buy_url']); ?>" target="_blank">
              <?php if(!empty($item['mainPic'])): ?><img src="<?php echo tpl_raw($item['mainPic']); ?>" alt="<?php echo tpl_raw($item['title']); ?>" loading="lazy"><?php else: ?><span class="gd-img-empty">惠</span><?php endif; ?>
            </a>
            <div class="gd-post-body">
              ...
              <div class="gd-post-meta">
                <span class="gd-post-hot">阅读：<em><?php echo tpl_raw($item['view']); ?></em></span>
