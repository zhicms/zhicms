...
/**
 * 站点业务控制器（照搬 kiees，仅 extends 改为 GuangdiuController）
 *
 * 访问：
 *   plug-guangdiu.html            -> index()   首页（商品推荐流 + 侧边热门榜）
 *   plug-guangdiu-<id>.html       -> detail()  详情
 *
 * 数据直接来自站点数据库：
 *   yun_article（推荐流/详情/热门榜，图片 mainPic、描述 dec、人气 view、购买链接 itemLink）
 *   yun_nav（顶部推荐类别导航）
 *   yun_items（优惠券，复用主站采集数据）
 * 大牌/风云榜复用主站大淘客接口 Tjk::getBrandList / getRankingList
 */
class SiteController extends GuangdiuController
{
    ...
    protected function buildSearchPager($page, $pageSize, $total, $kw, $filterType = '', $filterFrom = '')
    {
        ...
        if ($pages <= 1) return '';
        // 直接使用插件基链接（伪静态为 plug-guangdiu.html，动态含 ?），
        // 不再去掉 .html 后缀，避免翻页地址变成 plug-guangdiu?kw=.. 这种无效形式。
        $base = $this->plugUrl();
    ...
    public function detail($id)
    {
        ...
        if (empty($_COOKIE[$viewKey])) {
            $db->executeQuery("UPDATE `{pre}article` SET `view` = `view` + 1 WHERE `id` = ?", array($id));
    ...
    protected function plugUrl($mod = null, $id = null)
    {
        if ($id !== null) {
            ...
            // 伪静态：base 可能是 plug-guangdiu.html（含后缀），需去掉后再拼 -id.html
