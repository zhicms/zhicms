...
<main class="kc-main">
  <div class="kc-wrap">
    ...
    <div class="kc-content">
      <div class="kc-box">
        <div class="kc-sectitle"><h3>最新的推荐......</h3></div>
        <div class="kc-list">
          <?php foreach($list as $item): ?>
          <div class="kc-post">
            <a class="kc-post-img" href="<?php echo tpl_raw($item['buy_url']); ?>">
              <?php if(!empty($item['mainPic'])): ?>
              <img src="<?php echo tpl_raw($item['mainPic']); ?>" alt="<?php echo tpl_raw($item['title']); ?>" />
              <?php else: ?>
              <span class="kc-img-empty">值得买</span>
              <?php endif; ?>
            </a>
            <div class="kc-post-body">
              <h2 class="kc-post-title">
                <a href="<?php echo tpl_raw($item['buy_url']); ?>">
                  <?php echo tpl_raw($item['title']); if(!empty($item['dec'])): ?><span class="kc-price"> <?php echo tpl_raw($item['dec']); ?></span><?php endif; ?>
                </a>
              </h2>
              <div class="kc-post-detail"><?php if(!empty($item['content'])): ?><?php echo tpl_raw($item['content']); else: ?><?php echo tpl_raw($item['dec']); ?><?php endif; ?></div>
              <div class="kc-post-meta">
                <span class="kc-post-hot">推荐好物 - 人气：<em><?php echo tpl_raw($item['hot']); ?></em></span>
                <a class="kc-gobuy" href="<?php echo tpl_raw($item['buy_url']); ?>" rel="nofollow">查看详情>></a>
              </div>
            </div>
          </div>
          <?php endforeach; ?>
        </div>
        <?php echo tpl_raw($pager); ?>
      </div>
    </div>
