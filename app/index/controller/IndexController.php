...
class IndexController extends \app\base\controller\BaseController
{
    ...
    public function estimate(){
      ...
      $view['cateName'] = obj("index/global","controller")->Cid($view['cid']);
    ...
    private function resolveLinkCard($url) {
        ...
        return tcache($cacheKey, function() use ($url) {
            try {
                ...
                $item = $detail['data'];
                // 转链跳转URL：统一走 LinkController
                $buyUrl = url($route='index/link/jump/platform=<platform>/id=<id>', $params=array('platform'=>'tb', 'id'=>$goodsId));
    ...
    private function buildFallbackBtn($url) {
        ...
        if ($platform && $name) {
            ...
            $buyUrl = url($route='index/link/jump/platform=<platform>/id=<id>', $params=array('platform'=>$platform, 'id'=>$goodsId ?: urlencode($url)));
            $args = array(
                'title'      => $name . '好物推荐',
                'pic'        => '',
                'origPrice'  => 0,
