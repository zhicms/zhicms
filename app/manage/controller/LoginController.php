	...
	public function index(){
	   if(!\IS_POST){
        ...
        if (!$userName) {
            ...
            // 登录全局失败封禁（文件级，抗 cookie 重置爆破）：
            // 同 IP 或同用户名在 5 分钟内失败 ≥ 10 次，封禁 1 小时，期间拒绝登录。
            $clientIp = $this->getClientIp();
            $guard = new \ZhiCms\ext\LoginGuard();
            if ($guard->isBlocked($clientIp, $userName)) {
                $remain = ceil($guard->remaining($clientIp, $userName) / 60);
                echo json_encode(array("info" => "登录失败次数过多，已被临时封禁，请约 {$remain} 分钟后再试", "status" => "n"));
                exit;
            }
