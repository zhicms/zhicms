...
$config = [
    'version'   => '5.0.2', // 目标版本号
    'zipUrl'    => 'https://www.zhi.red/d/update/full_update_5.0.2.zip',
...
/**
 * 需要智能合并的配置文件（用户设置优先，避免更新覆盖后台保存的配置）
 */
...
/**
 * 智能合并配置文件：
 * - 通用配置（siteconfig/seo/sms/apiset/global）：array_merge($new, $old) 用户设置优先
 * - rule.php：REWRITE_ON 保持用户开关（伪静态开/关不被覆盖），REWRITE_RULE 合并（更新包新规则并入，用户规则保留）
 */
function mergeConfigFile($source, $dest) {
    ...
    // 读取变量名（$rule / $Siteinfo / $SEO / $api / $sms 等）
    $content = file_get_contents($dest);
    if (!preg_match('/\$(\w+)\s*=\s*array\(/', $content, $m)) {
        copy($source, $dest); // 无法解析则直接覆盖
        return;
    }
    $varName = $m[1];
    include $dest;      // 旧配置（用户当前）
    $oldConfig = isset($$varName) ? $$varName : array();
    include $source;    // 新配置（更新包）
    $newConfig = isset($$varName) ? $$varName : array();

    // rule.php 特殊处理：保留用户伪静态开关 + 合并规则
    if (basename($dest) === 'rule.php') {
        $merged = $newConfig;
        // REWRITE_ON 保持用户值（开/关不被更新包改变）
        if (isset($oldConfig['REWRITE_ON'])) {
            $merged['REWRITE_ON'] = $oldConfig['REWRITE_ON'];
        }
        // REWRITE_RULE 合并：以用户规则为基础，补入更新包新增规则（用户已有的不被覆盖）
        if (isset($oldConfig['REWRITE_RULE']) && is_array($oldConfig['REWRITE_RULE'])) {
            ...
            $merged['REWRITE_RULE'] = $newRules;          // 先取更新包最新规则
            foreach ($userRules as $k => $v) {
                $merged['REWRITE_RULE'][$k] = $v;         // 用户规则（含自定义/修改）覆盖
            }
            // 其他非规则键保持用户
            foreach ($oldConfig as $k => $v) {
                if ($k !== 'REWRITE_ON' && $k !== 'REWRITE_RULE' && !isset($merged[$k])) {
                    $merged[$k] = $v;
                }
            }
        } else {
            // 用户无规则（如首次），直接用更新包并保留开关
            foreach ($oldConfig as $k => $v) {
                if ($k !== 'REWRITE_ON' && $k !== 'REWRITE_RULE' && !isset($merged[$k])) {
                    $merged[$k] = $v;
                }
...
function copyFilesSafe($src, $dst, &$count = 0) {
    ...
    while (($file = readdir($dh)) !== false) {
        ...
        if (is_dir($srcPath)) {
            ...
            // 保护数据库配置：不覆盖
            if (isConfigFile($rel) && $file === 'db.php') continue;
            // 其他配置文件：智能合并（保留用户设置）
            if (isConfigFile($rel)) {
...
function executeDatabaseUpdate($sqlFile) {
    ...
    // 及 MySQL 原生错误码 1060 列已存在 / 1061 键已存在），
    // 其余错误仍抛出，避免旧库重复执行升级 SQL 时报 Duplicate column。
    // SQLSTATE 42S21/42S11（列已存在 / 键已存在）
    // 部分驱动对 ALTER 的重复列/重复索引报 SQLSTATE 42000 + 原生错误码 1060/1061，
    // 因此额外按“错误消息含 Duplicate column / Duplicate key”兜底判断。
    $skippable = array('42S21', '42S11', '1060', '1061'); // 列已存在 / 键已存在
    $skipped = 0;
